...
sci_normalized <- sci_normalized %>% 
  mutate(sci_wo_climate = round((added_indices - min_sci_added) /
...
sci_normalized_climate <- sci_normalized_climate %>% 
  mutate(sci_w_climate = round((added_indices - min_sci_clim_added) /
...
```{r}
press_bd <- left_join(pressure_normalized, biodiversity_normalized,
                      by = "id")

pbd_surf <- left_join(press_bd, surf_normalized,
                      by = "id") %>% 
  select(-join_count, -numerical_direction, -numerical_frequency, 
         -numerical_experience, -numerical_quality, -region)

pbdsurf_soc <- left_join(pbd_surf, social_normalized,
                         by = "id")

pbd_surfsoc_clim <- left_join(pbdsurf_soc, climate_normalized,
                              by = "id") %>% 
  select(-nm_regiao, -sum_10km, -hab_10km)

all_indices <- left_join(pbd_surfsoc_clim, response_normalized,
                         by = "id")

sci_normalized_last <- sci_normalized %>%
  mutate(sci_wo_clim_added_indices = added_indices) %>% 
  select(id, sci_wo_clim_added_indices, sci_wo_climate)

sci_wo_climate <- left_join(all_indices, sci_normalized_last,
                            by = "id")

sci_normalized_climate_last <- sci_normalized_climate %>% 
  mutate(sci_clim_added_indices = added_indices) %>% 
  select(id, sci_clim_added_indices, sci_w_climate)

both_scis <- left_join(sci_wo_climate, sci_normalized_climate_last,
                       by = "id")
```

# Merge with Surf Spot Names

```{r}
surf_spots <- read_csv(here("data", "surf_spot_states", "surf_spot_states.csv"))

all_data <- left_join(surf_spots, both_scis,
                      by = "id")
```
