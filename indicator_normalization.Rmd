...
built_area <- built_area %>% 
  mutate(percent_ba = round(sum_area_s / (shape_area / 1000000), 10))
...
```{r}
max_road <- round(max(roads$sum_length), 10)
min_road <- round(min(roads$sum_length), 10)

roads <- roads %>% 
  mutate(normalized_roads = round((sum_length - min_road) /
...
ba_mod_join <- left_join(built_area, human_mod, 
                         by = "objectid") %>% 
  select(-shape_area, -sum_area_s, -percent_ba, -mean)
...
surf_indicators <- surf_indicators %>% 
  mutate(normalized_clusters =  round((surf_indicators$join_count - min_cluster) / 
...
surf_indicators <- surf_indicators %>% 
  mutate(normalized_frequency = round((surf_indicators$numerical_frequency - min_frequency) / 
...
surf_indicators <- surf_indicators %>% 
  mutate(normalized_quality = round((surf_indicators$numerical_quality - min_quality) / 
