...
```{r}
write_csv(pressure_normalized, file = "se_normalized_pressure.csv")
```
...
coral <- coral %>% 
  ...
  # mutate(normalized_coral_reef = case_when(    ##### for se and s
...
```{r}
write_csv(biodiversity_normalized, file = "se_normalized_biodiversity.csv")
```
...
```{r}
write_csv(social_normalized, file = "se_normalized_social.csv")
```
...
```{r}
write_csv(climate_normalized, file = "se_normalized_climate.csv")
```
...
```{r}
write_csv(response_normalized, file = "se_normalized_response.csv")
```
