...
```{r}
# read in data
surf_indicators <- read_csv(here("data", "se", "surf", "se_surf_indicators.csv")) %>% 
...
```{r}
write_csv(surf_normalized, file = "se_normalized_surf.csv")
```

## Social Subindex

```{r}
employment <- read_csv(here("data", "se", "social", "se_employment.csv")) %>% 
  select(id, Join_Count, SLR_employment_per_capita) %>% 
  clean_names() %>% 
  mutate(slr_employ = slr_employment_per_capita) %>% 
  filter(id != 331 & id != 511)
hotels <- read_csv(here("data", "se", "social", "se_hotels.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
airports <- read_csv(here("data", "se", "social", "se_airports.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
```
...
```{r}
climate <- read_csv(here("data", "se", "climate", "se_climate.csv")) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511) %>% 
  select(nm_regiao, id, sum_10km, hab_10km) %>% 
  filter(nm_regiao != "Sul", nm_regiao != "Norte", nm_regiao != "Nordeste")
```
...
```{r}
heritage <- read_csv(here("data", "se", "response", "se_world_heri.csv")) %>% 
  select(id, world_heritage_percent_area) %>% 
  clean_names() %>% 
  mutate(heritage_pa = world_heritage_percent_area) %>% 
  select(-world_heritage_percent_area) %>% 
  filter(id != 331 & id != 511)
atl_for_priority <- read_csv(here("data", "se", "response", "se_PAF.csv")) %>% 
  select(id, protected_atl_forest_area) %>% 
  clean_names() %>% 
  mutate(atl_for_pa = protected_atl_forest_area) %>% 
  select(-protected_atl_forest_area) %>% 
  filter(id != 331 & id != 511)
guarda <- read_csv(here("data", "se", "response", "se_wsr.csv")) %>% 
  select(id, wsr_percent_area) %>% 
  clean_names() %>% 
  mutate(wsr_pa = wsr_percent_area) %>% 
  select(-wsr_percent_area) %>% 
  filter(id != 331 & id != 511)
prot_areas <- read_csv(here("data", "se", "response", "se_protected.csv")) %>% 
  select(id, protected_percent_area) %>% 
  clean_names() %>% 
  mutate(protected = protected_percent_area) %>% 
  filter(id != 331 & id != 511)
bandeira <- read_csv(here("data", "se", "response", "se_bandeira.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
ramsar <- read_csv(here("data", "se", "response", "se_ramsar.csv")) %>% 
  select(id, ramsar_percent_area) %>% 
  clean_names() %>% 
...
```{r}
write_csv(sci_normalized, file = "se_sci_wo_climate.csv")
```
...
```{r}
write_csv(sci_normalized_climate, file = "se_sci_w_climate.csv")
```
