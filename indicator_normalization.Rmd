...
```{r}
# Test for github to see if it works!!
```
...
```{r}
# read in data
surf_indicators <- read_csv(here("data", "nne", "surf", "nne_surf_spots.csv")) %>% 
...
climate <- climate %>% 
  mutate(normalized_cp = round((hab_10km - min_cp) / (max_cp - min_cp), 10))
```
...
climate_normalized <- climate_normalized %>% 
  mutate(climate_index = round((climate_normalized$climate_added - min_climate_added) / (max_climate_added - min_climate_added), 10))
```
...
press_biod_surf_soc_clim <- left_join(press_biod_surf_soc, climate_normalized,
                                 by = "id") %>% 
  select(id, pressure_index, biodiversity_subindex,
         surf_subindex, social_subindex, climate_index)

all_indices <- left_join(press_biod_surf_soc_clim, response_normalized,
                         by = "id") %>% 
  select(id, pressure_index, biodiversity_subindex,
         surf_subindex, social_subindex, climate_index,
         response_index)
...
```{r}
write_csv(sci_normalized, file = "sci_w_climate.csv")
```
