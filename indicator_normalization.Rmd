...
```{r}
nne_built_area <- read_csv(here("data", "nne", "pressure", "nne_built_area.csv")) %>% 
  select(id, Shape_Area, SUM_Area_S) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  select(-sum_area_s)
nne_human_mod <- read_csv(here("data", "nne", "pressure", "nne_human_mod.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names()
nne_pop <- read_csv(here("data", "nne", "pressure", "nne_pop.csv")) %>% 
  select(id, pop_change) %>% 
  clean_names()
nne_ports <- read_csv(here("data", "nne", "pressure", "nne_ports.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names()
nne_roads <- read_csv(here("data", "nne", "pressure", "nne_roads.csv")) %>% 
...
```{r}
nne_built_area <- nne_built_area %>% 
  mutate(percent_ba = round(sum_area_sqkm / (shape_area / 1000000), 10))

max_percent_ba <- round(max(nne_built_area$percent_ba), 10)
min_percent_ba <- round(min(nne_built_area$percent_ba), 10)
...
```{r}
max_human_mod <- round(max(nne_human_mod$mean), 10)
min_human_mod <- round(min(nne_human_mod$mean), 10)

nne_human_mod <- nne_human_mod %>% 
...
```{r}
max_pop_change <- round(max(nne_pop$pop_change), 10)
min_pop_change <- round(min(nne_pop$pop_change), 10)
...
```{r}
max_ports <- round(max(nne_ports$join_count), 10)
min_ports <- round(min(nne_ports$join_count), 10)
...
```{r}
max_road <- round(max(nne_roads$sum_length_km), 10)
min_road <- round(min(nne_roads$sum_length_km), 10)
...
```{r}
nne_ba_mod_join <- left_join(nne_built_area, nne_human_mod, 
                         by = "id") %>% 
  select(-shape_area, -sum_area_sqkm, -percent_ba, -mean)

nne_ba_mod_pop <- left_join(nne_ba_mod_join, nne_pop,
                        by = "id") %>% 
  select(-pop_change)

nne_ba_mod_pop_ports <- left_join(nne_ba_mod_pop, nne_ports,
                              by = "id") %>% 
  select(-join_count)

nne_pressure_normalized <- left_join(nne_ba_mod_pop_ports, nne_roads,
                                 by = "id") %>% 
...
```{r}
nne_pressure_normalized <- nne_pressure_normalized %>% 
  mutate(added_pressure = round(normalized_built_area + normalized_human_mod +
                                  normalized_pop_change + normalized_ports +
                                  normalized_roads, 10))

max_pressure <- round(max(nne_pressure_normalized$added_pressure), 10)
min_pressure <- round(min(nne_pressure_normalized$added_pressure), 10)
...
```{r}
write_csv(nne_pressure_normalized, file = "nne_normalized_pressure.csv")
```
...
```{r}
write_csv(climate_normalized, file = "normalized_climate.csv")
```
