...
```{r}
built_area <- read_csv(here("data", "se", "pressure", "se_built_area.csv")) %>% 
  select(id, Shape_Area, sum_Area_SQUAREKILOMETERS) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_squarekilometers) %>% 
  select(-sum_area_squarekilometers) %>% 
  filter(id != 331 & id != 511)
human_mod <- read_csv(here("data", "se", "pressure", "se_human_mod.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
pop <- read_csv(here("data", "se", "pressure", "se_pop.csv")) %>% 
  select(id, pop_change) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
ports <- read_csv(here("data", "se", "pressure", "se_ports.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
roads <- read_csv(here("data", "se", "pressure", "se_roads.csv")) %>% 
  select(id, sum_Length_KILOMETERS) %>% 
  clean_names() %>% 
...
```{r}
marine_richness <- read_csv(here("data", "se", "biodiversity", "se_mar_rich.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
terr_richness <- read_csv(here("data", "se", "biodiversity", "se_terr_rich.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
coral <- read_csv(here("data", "se", "biodiversity", "se_coral.csv")) %>% 
  select(id, coral_reef_area) %>% 
  clean_names() %>% 
  mutate(coral_reef_pa = coral_reef_area) %>% 
  select(-coral_reef_area) %>% 
  filter(id != 331 & id != 511)
mangrove <- read_csv(here("data", "se", "biodiversity", "se_mangrove.csv")) %>% 
  select(id, mangrove_percent_area) %>% 
  clean_names() %>% 
  mutate(mangrove_pa = mangrove_percent_area) %>% 
  select(-mangrove_percent_area) %>% 
  filter(id != 331 & id != 511)
seagrass <- read_csv(here("data", "se", "biodiversity", "se_seagrass.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
tree_cover <- read_csv(here("data", "se", "biodiversity", "se_tree_cover.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
terr_pa_high <- read_csv(here("data", "se", "biodiversity", "se_terr_pa_high.csv")) %>% 
  select(id, high_percent_area) %>% 
  clean_names() %>% 
  mutate(high_pa = high_percent_area) %>% 
  select(-high_percent_area) %>% 
  filter(id != 331 & id != 511)
terr_pa_ver_high <- read_csv(here("data", "se", "biodiversity", "se_terr_pa_v_high.csv")) %>% 
  select(id, very_high_percent_area) %>% 
  clean_names() %>% 
  mutate(very_high_pa = very_high_percent_area) %>% 
  select(-very_high_percent_area) %>% 
  filter(id != 331 & id != 511)
terr_pa_ext_high <- read_csv(here("data", "se", "biodiversity", "se_terr_pa_ext_high.csv")) %>% 
  select(id, extremely_high_percent_area) %>% 
  clean_names() %>% 
  mutate(ext_high_pa = extremely_high_percent_area) %>% 
  select(-extremely_high_percent_area) %>% 
  filter(id != 331 & id != 511)
ocean_pa <- read_csv(here("data", "se", "biodiversity", "se_oceanic_pa.csv")) %>% 
  select(id, oceanic_pa_percent_area) %>% 
...
coral <- coral %>% 
  ...
  mutate(normalized_coral_reef = case_when(
