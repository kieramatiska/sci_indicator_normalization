...
```{r}
built_area <- read_csv(here("data", "nne", "pressure", "nne_built_area.csv")) %>% 
  select(id, Shape_Area, SUM_Area_S) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  select(-sum_area_s) %>% 
  filter(id != 331 & id != 511)
human_mod <- read_csv(here("data", "nne", "pressure", "nne_human_mod.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
pop <- read_csv(here("data", "nne", "pressure", "nne_pop.csv")) %>% 
  select(id, pop_change) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
ports <- read_csv(here("data", "nne", "pressure", "nne_ports.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
roads <- read_csv(here("data", "nne", "pressure", "nne_roads.csv")) %>% 
...
```{r}
write_csv(pressure_normalized, file = "nne_normalized_pressure.csv")
```
...
```{r}
marine_richness <- read_csv(here("data", "nne", "biodiversity", "nne_marine_rich.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
terr_richness <- read_csv(here("data", "nne", "biodiversity", "nne_terr_rich.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
coral <- read_csv(here("data", "nne", "biodiversity", "nne_coral_reef.csv")) %>% 
  select(id, coral_reef) %>% 
  clean_names() %>% 
  mutate(coral_reef_pa = coral_reef) %>% 
  select(-coral_reef) %>% 
  filter(id != 331 & id != 511)
mangrove <- read_csv(here("data", "nne", "biodiversity", "nne_mangrove.csv")) %>% 
  select(id, mangrove_p) %>% 
  clean_names() %>% 
  mutate(mangrove_pa = mangrove_p) %>% 
  select(-mangrove_p) %>% 
  filter(id != 331 & id != 511)
seagrass <- read_csv(here("data", "nne", "biodiversity", "nne_seagrass.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
tree_cover <- read_csv(here("data", "nne", "biodiversity", "nne_tree_cover.csv")) %>% 
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
terr_pa_high <- read_csv(here("data", "nne", "biodiversity", "nne_terr_pa_high.csv")) %>% 
  select(id, high_perce) %>% 
  clean_names() %>% 
  mutate(high_pa = high_perce) %>% 
  select(-high_perce) %>% 
  filter(id != 331 & id != 511)
terr_pa_ver_high <- read_csv(here("data", "nne", "biodiversity", "nne_terr_pa_vh.csv")) %>% 
  select(id, very_high_) %>% 
  clean_names() %>% 
  mutate(very_high_pa = very_high) %>% 
  select(-very_high) %>% 
  filter(id != 331 & id != 511)
terr_pa_ext_high <- read_csv(here("data", "nne", "biodiversity", "nne_terr_pa_ext_high.csv")) %>% 
  select(id, extremely_) %>% 
  clean_names() %>% 
  mutate(ext_high_pa = extremely) %>% 
  select(-extremely) %>% 
  filter(id != 331 & id != 511)
ocean_pa <- read_csv(here("data", "nne", "biodiversity", "nne_oceanic_pa.csv")) %>% 
  select(id, oceanic_pa) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
```
...
```{r}
write_csv(biodiversity_normalized, file = "nne_normalized_biodiversity.csv")
```
...
```{r}
# read in data
surf_indicators <- read_csv(here("data", "nne", "surf", "nne_surf_indicators.csv")) %>% 
  select(id, Join_Count, numerical_direction, 
         numerical_frequency, numerical_experience, numerical_quality) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
  # select(id, Join_Count, numerical_, numerical1, numerica_1, numerica_2) %>% 
  # clean_names() %>% 
...
```{r}
write_csv(surf_normalized, file = "nne_normalized_surf.csv")
```

## Social Subindex

```{r}
employment <- read_csv(here("data", "nne", "social", "nne_employment.csv")) %>% 
  select(id, Join_Count, SLR_employ) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
hotels <- read_csv(here("data", "nne", "social", "nne_hotels.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
airports <- read_csv(here("data", "nne", "social", "nne_airports.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
```
...
```{r}
write_csv(social_normalized, file = "nne_normalized_social.csv")
```

# Climate Index

```{r}
climate <- read_csv(here("data", "nne", "climate", "climate.csv")) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511) %>% 
  select(nm_regiao, id, sum_10km, hab_10km) %>% 
  filter(nm_regiao != "Sul", nm_regiao != "Sudeste")
```
...
```{r}
write_csv(climate_normalized, file = "nne_normalized_climate.csv")
```

# Response Index

```{r}
heritage <- read_csv(here("data", "nne", "response", "nne_world_heritage.csv")) %>% 
  select(id, SUM_Area_S, world_heri) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  mutate(heritage_pa = world_heri) %>% 
  select(-sum_area_s, -world_heri) %>% 
  filter(id != 331 & id != 511)
atl_for_priority <- read_csv(here("data", "nne", "response", "nne_PAF.csv")) %>% 
  select(id, SUM_Area_S, protected_) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  mutate(atl_for_pa = protected) %>% 
  select(-sum_area_s, -protected) %>% 
  filter(id != 331 & id != 511)
guarda <- read_csv(here("data", "nne", "response", "nne_wsr.csv")) %>% 
  select(id, SUM_Area_S, wsr_percen) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  mutate(wsr_pa = wsr_percen) %>% 
  select(-sum_area_s, -wsr_percen) %>% 
  filter(id != 331 & id != 511)
prot_areas <- read_csv(here("data", "nne", "response", "nne_protected.csv")) %>% 
  select(id, SUM_Area_S, protected_) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  filter(id != 331 & id != 511)
bandeira <- read_csv(here("data", "nne", "response", "nne_bandeira.csv")) %>% 
  select(id, Join_Count) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
ramsar <- read_csv(here("data", "nne", "response", "nne_ramsar.csv")) %>% 
  select(id, SUM_Area_S, ramsar_per) %>% 
  clean_names() %>% 
  mutate(sum_area_sqkm = sum_area_s) %>% 
  mutate(ramsar_pa = ramsar_per) %>% 
  select(-sum_area_s, -ramsar_per) %>% 
  filter(id != 331 & id != 511)
```
...
guarda <- guarda %>% 
  mutate(normalized_wsr = round((wsr_pa - min_wsr) / 
                          (max_wsr - min_wsr), 10)) %>% 
  mutate(normalized_wsr = case_when( # only for the regions with Guarda's surfing reserve
...
```{r}
write_csv(response_normalized, file = "nne_normalized_response.csv")
```
...
```{r}
write_csv(sci_normalized, file = "nne_sci_wo_climate.csv")
```
...
sci_normalized_climate <- sci_normalized_climate %>% 
  mutate(sci = round((added_indices - min_sci_clim_added) /
                       (max_sci_added - min_sci_clim_added), 10))
```

## Export SCI (w/ climate)

```{r}
write_csv(sci_normalized_climate, file = "nne_sci_w_climate.csv")
```
