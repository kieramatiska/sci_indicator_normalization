...
human_mod <- read_csv(here("data", "pressure", "human_mod.csv")) %>% 
  filter(id >= 93 & id <= 194 | id >= 445 & id <= 510 | id >= 537 & id <=548) %>% 
...
marine_richness <- read_csv(here("data", "biodiversity", "marine_rich.csv")) %>%
  filter(id >= 93 & id <= 194 | id >= 445 & id <= 510 | id >= 537 & id <=548) %>% # for Northern and Northeastern region
  # filter(id >= 0 & id <= 8 | id >= 195 & id <= 328 | id >= 512 & id <= 518) %>% # for Southern region
  select(id, MEAN) %>% 
  clean_names() %>% 
  filter(id != 331 & id != 511)
...
seagrass <- seagrass %>% 
  ...
  #mutate(normalized_seagrass = case_when(    ##### for s
...
terr_pa_high <- terr_pa_high %>% 
  ...
  #mutate(normalized_terr_pa_high = case_when(    ##### for s
...
guarda <- guarda %>% 
  mutate(normalized_wsr = round((wsr_pa - min_wsr) / 
                          (max_wsr - min_wsr), 10)) %>% 
  mutate(normalized_wsr = case_when( # only for the regions without Guarda's surfing reserve
