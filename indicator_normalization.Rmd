...
employment_normalized <- round((employment$slr_employ - min_employment) /
                                         (max_employment - min_employment), 10)

employment$normalized_employment <- employment_normalized
```
...
hotels_normalized <- round((hotels$join_count - min_hotel) /
                                     (max_hotel - min_hotel), 10)

hotels$normalized_hotels <- hotels_normalized
```
...
```{r}
social_normalized <- merge(employment$normalized_employment, hotels$normalized_hotels,
                           by = "objectid")

employment %>% 
  left_join(hotels, by = "objectid")
